...
// Position log data structure (based on the example JSON)
export interface PositionLogData {
    ...
    fingerPositions: Array<{
        ...
        detectedFingers: {
            thumb: { position: string; confidence: number; coordinates: { x: number; y: number; z: number } };
            index: { position: string; confidence: number; coordinates: { x: number; y: number; z: number } };
            middle: { position: string; confidence: number; coordinates: { x: number; y: number; z: number } };
            ring: { position: string; confidence: number; coordinates: { x: number; y: number; z: number } };
            pinky: { position: string; confidence: number; coordinates: { x: number; y: number; z: number } };
        };
        analysis: {
            correctFinger: boolean;
    ...
    gameResults: {
        ...
        maxCombo: number;
        fingerPerformance: {
            finger1: { hits: number; misses: number; accuracy: number };
            finger2: { hits: number; misses: number; accuracy: number };
            finger3: { hits: number; misses: number; accuracy: number };
            finger4: { hits: number; misses: number; accuracy: number };
            finger5: { hits: number; misses: number; accuracy: number };
        };
        timing: {
            startTime: string;
            endTime: string;
    ...
    metadata: {
        ...
        calibration: {
            ...
            timingTolerancePerfect: number;
            timingToleranceGood: number;
 ...
 */
export const convertPositionLogToFingerDanceResult = (positionLogData: PositionLogData): Omit<FingerDanceResult, 'id'> => {
    // Convert finger positions to finger movements
    const fingerMovementLog: FingerMovement[] = positionLogData.fingerPositions.map(position => {
        // Determine which fingers were actually detected as "down"
        const actualFingers: number[] = [];
        const detectedFingers = position.detectedFingers;
        
        if (detectedFingers.thumb.position === 'down') actualFingers.push(1);
        if (detectedFingers.index.position === 'down') actualFingers.push(2);
        if (detectedFingers.middle.position === 'down') actualFingers.push(3);
        if (detectedFingers.ring.position === 'down') actualFingers.push(4);
        if (detectedFingers.pinky.position === 'down') actualFingers.push(5);
    ...
    const result: Omit<FingerDanceResult, 'id'> = {
        gameType: 'fingerDance',
 ...
 */
export const validatePositionLogData = (data: any): data is PositionLogData => {
    try {
 ...
 */
export const parsePositionLogFromJson = (jsonString: string): PositionLogData | null => {
    try {
        const data = JSON.parse(jsonString);
        if (validatePositionLogData(data)) {
            return data;
        }
        return null;
    } catch (error) {
 ...
 */
export const loadPositionLogFromFile = async (file: File): Promise<Omit<FingerDanceResult, 'id'> | null> => {
    try {
        const fileContent = await file.text();
        const positionLogData = parsePositionLogFromJson(fileContent);
        
        if (positionLogData) {
            return convertPositionLogToFingerDanceResult(positionLogData);
        }
 ...
 */
export const generateSamplePositionLog = (sessionID: string, patientID: string): PositionLogData => {
    return {
        sessionInfo: {
            sessionID,
            patientID,
