...
function SessionDetailCard({ session, gameResult, romData }: { session: Session; gameResult: GameResult | undefined; romData: Rom | null }) {
    ...
    const GameDetails = () => {
        ...
        if (gameResult.gameType === 'appleGame') {
    ...
    return (
        <Card
            className={`transition-all duration-300 overflow-hidden ${isExpanded ? 'shadow-md border-primary' : 'shadow-sm'}`}
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent
                ...
                style={{
                    width: 'calc(1.8 * 90vh)', // Genişlik = Yüksekliğin 2 katı (90vh * 2)
                    maxWidth: '95vw' // Ekran genişliğini aşmasını engelle
                }}
            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 flex-grow overflow-hidden p-6 bg-gray-50/50 dark:bg-black/20">
                    {/* Sol Panel: Bilgiler */}
